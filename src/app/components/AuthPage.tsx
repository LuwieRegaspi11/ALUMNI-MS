...
function SignInForm({ onToast }: { onSwitch: () => void; onToast: (m: string) => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-start h-full px-8 py-6 overflow-y-auto">
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute top-0 left-0 w-1/2 h-full" style={{ zIndex: 2 }}>
          <SignInForm onSwitch={toggle} onToast={showToast} />
        </div>

        {/* ── Sign Up form — RIGHT half (always rendered) ── */}
        <div className="absolute top-0 right-0 w-1/2 h-full" style={{ zIndex: 2 }}>
